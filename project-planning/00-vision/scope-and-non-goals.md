...
2. "Easy to add later" is the default posture: prefer architecture that _leaves room_ (config-driven site data, content collections, token-based design) over architecture that _pre-builds_ unrequested features.
...
The architecture must _accommodate_ these without building them (config-driven site data, MDX collections, token theming, no lock-in to Vercel-only APIs — D-016).
