...
- `project-planning/` is the **single source of truth** for *what* this website is, *why*, and *how* it is built. The website code lives at the repo root, outside this folder.
...
| `05-roadmap/` | Roadmap + task breakdown (P-*, X-*) | Task statuses updated continuously; add tasks, never delete (mark `deferred` with reason) |
...
2. **Change to an approved decision** → new entry *superseding* the old one; never edit an Approved entry's outcome in place (annotate "Superseded by D-XXX").
...
3. Canonical facts (README §3) are referenced by ID (D-*, A-*, F-*, P-*, G-*) — never paraphrase silently; keep IDs unique.
