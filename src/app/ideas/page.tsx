...
export const metadata: Metadata = {
  title: "Ideas",
  description: "Thoughts, observations, and concepts — not client or project writing.",
      ...
      <Section className="pt-0">
        {ideas.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {ideas.map((idea) => (
              <Link key={idea.slug} href={`/ideas/${idea.slug}`} className="group block h-full">
                <Card padded={false} className="flex h-full flex-col overflow-hidden transition-colors group-hover:border-accent">
                  <CoverImage src={idea.cover} alt="" className="aspect-[3/2] w-full object-cover" />
                  <div className="flex flex-1 flex-col gap-3 p-6 md:p-8">
                    ...
                    <h2 className="text-xl font-semibold group-hover:underline">{idea.title}</h2>
                    <p className="text-muted">{idea.summary}</p>
                    <p className="text-faint mt-auto text-sm">{formatDate(idea.published)}</p>
                  </div>
                </Card>
              </Link>
            ))}
